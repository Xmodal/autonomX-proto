    ...
    if(ConstParams::Number_Of_Connection != ConstParams::Number_Of_Neurons)
    {
        for(int src_id=0; src_id<Number_Of_Neurons; ++src_id){
            ...
            while(count < ConstParams::Number_Of_Connection){
                ...
                // possible bug here, should also prevent cases where dest = source?
            ...
            for(int j=0; j<ConstParams::Number_Of_Connection; j++){
                ...
                if(dest_id != src_id){
                    ...
                    // in the case of non-fully connected networks, using an weight matrix is inefficient. the sparser, the worst, because all possible connections contributions (neuron pairs) are computed anyways.
...
void SpikingNet::setGridNetwork(){
    ...
    for(int i=0; i<Number_Of_Neurons; ++i) {
        ...
        for(int j=0; j<Number_Of_Neurons; j++){
            ...
            } else if ( i != j && (abs(row - row_target) <= 1 && (abs(col - col_target) <= 1  || abs(col - col_target) == 19)) ) {
                weights[i][j] = Init_Weight_Ratio_Exc*rand_uni(rand_gen);  // HERE!!!
            } else {
                weights[i][j] = 0;
            }
